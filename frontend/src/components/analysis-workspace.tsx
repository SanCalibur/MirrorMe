...
type Metric = { key: string; label: string; score: number; detail: string; evidence?: string[] }
type Assessment = {
  id: string
  date: string
  version: number
...
function scoreClass(key: string, value: number) { const good = key === "pressure_load" ? 100 - value : value; return good >= 72 ? "text-emerald-700" : good >= 48 ? "text-amber-700" : "text-rose-700" }
...
export function AnalysisWorkspace() {
  ...
  const load = async (start = startDate, end = endDate) => {
    setStatus("loading")
    try {
      const next = await getJson<Assessment[]>(`/api/state-assessments?method=llm&latest_per_day=1&start_date=${start}&end_date=${end}`)
      setRecords(next)
  ...
  useEffect(() => {
    ...
    void getJson<Assessment[]>(`/api/state-assessments?method=llm&date=${selectedDate}`).then(setVersions).catch(() => setVersions([]))
  }, [selectedDate])
  ...
  return <>
    ...
    {status === "loading" ? <p className="py-20 text-sm text-zinc-500">正在读取观察记录...</p> : status === "error" ? <p className="py-20 text-sm text-rose-700">读取失败：{error}</p> : !records.length ? <section className="py-20"><p className="text-xl font-semibold">这个区间还没有状态观察。</p><p className="mt-3 text-sm text-zinc-600">先在数据采集页清洗或确认当天文本，再留存一次观察。</p></section> : <>
      <section className="grid border-b border-zinc-200 py-8 lg:grid-cols-[1fr_18rem] lg:gap-12"><div><p className="text-sm font-medium text-zinc-500">所选日期</p><h2 className="mt-1 text-2xl font-semibold tracking-tight">{selected && labelDate(selected.date, true)}</h2><p className="mt-6 max-w-2xl text-lg leading-8 text-zinc-800">{selected?.assessment.summary}</p><p className="mt-4 text-sm text-zinc-600">{selected?.assessment.data_quality === "limited" ? "样本有限，建议结合后续多日观察。" : "样本量可用于当日文本观察。"} 置信度 {Math.round((selected?.assessment.confidence ?? 0) * 100)}%，依据 {selected?.assessment.source_event_count ?? 0} 条输入记录。</p></div><div className="mt-8 border-l-2 border-emerald-700 pl-4 lg:mt-1"><p className="text-xs font-semibold uppercase tracking-[.14em] text-emerald-700">LLM observation</p><p className="mt-3 text-sm leading-6 text-zinc-700">模型按固定 JSON 结构生成观察与证据。它不是诊断，也不应被当作对人格或健康状态的判断。</p></div></section>
