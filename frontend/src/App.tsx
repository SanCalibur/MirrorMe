import { BarChart3, BookOpenText, CalendarRange, Database, KeyRound, Menu, Presentation, Settings2, SlidersHorizontal, X } from "lucide-react"
...
const pages = [{ href: "/capture", label: "数据采集", icon: Database }, { href: "/analysis", label: "数据解读", icon: BarChart3 }, { href: "/review", label: "周回顾", icon: CalendarRange }, { href: "/diary", label: "日记", icon: BookOpenText }, { href: "/showcase", label: "MVP 展示", icon: Presentation }, { href: "/settings", label: "设置", icon: Settings2 }]
...
function Diary() { return <Shell><DiaryWorkspace /></Shell> }

function WeeklyReview() { return <Shell><WeeklyReviewWorkspace /></Shell> }
...
export default function App() { const path = window.location.pathname; return path === "/analysis" || path === "/state" ? <Analysis /> : path === "/review" ? <WeeklyReview /> : path === "/diary" ? <Diary /> : path === "/showcase" ? <Showcase /> : path === "/settings" ? <BatchSettings /> : <Capture /> }
